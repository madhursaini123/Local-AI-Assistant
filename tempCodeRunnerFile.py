...
# Load .env even if it's in the "dotenv" folder
dotenv_path = Path(__file__).parent / "dotenv" / ".env"
if dotenv_path.exists():
    load_dotenv(dotenv_path)
else:
    # fallback to look in root folder
    load_dotenv()
...
def llama_chat_with_history(user_input):
    ...
    try:
        ...
        if rasp.status_code != 200:
            return f"Error: {rasp.status_code} - {rasp.text}"
